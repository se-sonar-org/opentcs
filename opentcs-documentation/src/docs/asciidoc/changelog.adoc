...
*** Add a `POST /vehicles/{NAME}/commAdapter/position` endpoint for setting (or resetting) a vehicle's position via its communication adapter.
    This provides the same functionality as the "Set position" action for a vehicle's driver in the openTCS Kernel Control Center, which is supported by the loopback communication adapter.
* Bugs fixed:
