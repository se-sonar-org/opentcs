...
*** Add endpoint `POST /vehicles/{NAME}/commAdapter/position` for setting the position reported by a vehicle's driver (equivalent to setting the position via the "Current position/state" panel of the vehicle driver window in the Kernel Control Center).
* Bugs fixed:
